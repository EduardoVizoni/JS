 ...
 */
function controleDeCompras(itens) {
    if (!Array.isArray(itens) || itens.length === 0 || !itens.every(item => item.nome && typeof item.quantidade === 'number' && typeof item.preco === 'number')) {
        return null;
    }
    ...
    return {totalGasto, itemMaisCaro, listaItens};
    console.log(controleDeCompras[null]);
}
